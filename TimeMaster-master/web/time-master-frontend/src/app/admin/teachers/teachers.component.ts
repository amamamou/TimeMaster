...
export class TeachersComponent implements OnInit {
  ...
  constructor(
    ...
    this.editTeacherForm = this.fb.group({
      ...
      email:  ['', Validators.required] // Assuming a string input for simplicity
    });
  ...
  startEditing(teacher: Teacher): void {
    ...
    this.editTeacherForm.patchValue({
      ...
      assignedActivities: teacher.assignedActivities,
      email: teacher.email, // Assuming comma-separated values for display
    });
  }
  ...
  onSubmit(): void {
    if (this.teacherForm.valid) {
      this.teachersService.addTeacher(this.teacherForm.value).subscribe((teacher) => {
        this.teachers.push(teacher); // Ajouter le nouveau enseignant à la liste
        this.teacherForm.reset(); // Réinitialiser le formulaire
      });
    }
  }
  clearForm(): void {
